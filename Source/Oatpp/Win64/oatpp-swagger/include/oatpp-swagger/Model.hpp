...
public:
  ...
  private:
    ...
    {
      auto temp_header = getHeader();
      if(!temp_header->contact)
      {
          temp_header->contact = Contact::createShared();
      }
      return temp_header->contact;
    }
    
    std::shared_ptr<License> getLicense()
    {
      auto temp_header = getHeader();
      if(!temp_header->license)
      {
          temp_header->license = License::createShared();
      }
      return temp_header->license;
    }
